...
def _dispatch_nudge(payload: dict, world: dict, session_id: str) -> str:
    ...
    dirs = [Path(world.get("nudges_dir") or ""), paths.builtin_nudges_dir()]
    if not any(str(d) and d.is_dir() for d in dirs):
...
def _bump_lesson_deliveries(world_name: str, path: Path) -> None:
    try:
...
def _pending_lessons(world_name: str, session_id: str, cwd, limit: int) -> list[dict]:
    ...
    for obj in chosen:
        try:
            ...
            pass
        _bump_lesson_deliveries(world_name, obj["_path"])
...
def _maybe_kick_worker(snapshot: dict) -> None:
    ...
    if shutil.which("uv") is None:
        return
    if paths.worker_lock_file().exists():
        return
...
def _handle_user_prompt_submit(payload: dict, world: dict, snapshot: dict) -> str:
    ...
    lessons = _pending_lessons(world_name, session_id, cwd, limit=2)
    if lessons:
        parts.append("\n".join(_format_lesson(l) for l in lessons))
...
def _handle_post_tool_use(payload: dict, world: dict, snapshot: dict) -> str:
    ...
    if tool_name == "Skill":
        usage.append_event(paths.usage_events_file(), {
            ...
            "detail": {"args": str(tool_input.get("args", ""))[:200]},
        })
...
def _truncate_value(v, limit: int = 500):
    s = v if isinstance(v, (str, int, float, bool)) or v is None else json.dumps(v, default=str)
...
def _handle_subagent_stop(payload: dict, world: dict, snapshot: dict) -> str:
    ...
    skip_keys = {"transcript", "transcript_content", "messages", "content"}
    detail = {k: _truncate_value(v) for k, v in payload.items() if k not in skip_keys}
    ...
    try:
        ...
        if handler is not None:
            try:
                ...
                _log(f"{event} handler failed: {e!r}")
                text = ""
    except Exception as e:
        _log(f"hook invocation failed: {e!r}")
        text = ""
