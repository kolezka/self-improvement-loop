...
function startGitHead(sessionId) {
  try {
    ...
    return isRecord7(parsed) ? str3(parsed["git_head"]) : null;
  } catch {
    return null;
  }
}
...
`)) {
    ...
    if (rtype === "attachment") {
      ...
      const attachment = isRecord8(attachmentRaw) ? attachmentRaw : {};
...
function handlePostToolUse(payload, world) {
  ...
  const toolInput = isRecord9(toolInputRaw) ? toolInputRaw : {};
  if (toolName === "Skill") {
    appendUsageEvent(usageEventsFile(), {
      ts: nowIso(),
      session_id: sessionId,
...
var usageFailureLogged = false, HOOK_RUNS_ROTATE_AT_BYTES, HOOK_RUNS_KEEP_LINES = 40000, hookRunFailureLogged = false, sampleFailureLogged = false, RULE_TAG_RE, SESSION_END_LOCK_MS = 4000, HANDLERS;
